...
# Calling some additional functions here to make this work piecemeal
from fil2dat import get_blc
from fil2dat import sort_by_blc
...
def find_event_pipeline(dat_file_list_str,
    ...
    source_name_list = []
    for item in dat_file_list:
        # adding some extra lines to deal with the piecemeal fil files
        file_id = get_blc(item)
        datlist_str = datdir + 'dat' + file_id + '_files.lst'
        datlist = open(datlist_str).readlines()
        datlist = [files.replace('\n','') for files in datlist]
        datlist = [files.replace(',','') for files in datlist]
        for dat in datlist:
    ...
    for i in range((int(n_files/number_in_cadence))):
        ...
        if not complex_cadence:
            if on_off_first == 'ON':
                name = file_sublist[0].split('_')[0][-6:]  
                id_num = (file_sublist[0].split('_')[0]).split('hits')[1]
            if on_off_first == 'OFF':
                name = file_sublist[1].split('_')[0][-6:]  
                id_num = (file_sublist[1].split('_')[0]).split('hits')[1]
        else:
            name = file_sublist[complex_cadence.index(1)].split('_')[0][-6:]  
            id_num = (file_sublist[complex_cadence.index(1)].split('_')[0]).split('hits')[1]
